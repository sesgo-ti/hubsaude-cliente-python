...
- ``access_token`` ausente ou vazio é erro (``SmartTokenError``).
...
#: Mantido local (não em defaults.py): ``discovery.py`` (Tarefa 3/B6, já
#: concluída) lê o corpo da resposta de descoberta via
#: ``response.json()`` diretamente, sem limite de tamanho em streaming --
#: não é um segundo consumidor deste valor. Continua sem outro
#: colaborador da lib que precise do mesmo limite; se isso mudar, mover
#: para defaults.py nesse momento (mesmo critério já aplicado aqui a
#: DEFAULT_EXPIRES_IN_SECONDS, que foi para defaults.py por já nascer
...
class TokenResponse:
    """Resposta de sucesso do token endpoint, já validada e sanitizada.
...
class TokenResponseGuard:
    ...
    def read_body(self, response: httpx.Response, trace: TraceContext) -> bytes:
        """Lê o corpo da resposta em streaming, interrompendo assim que
        ...
        Args:
            response: resposta do token endpoint (idealmente obtida com
                ...
                também sobre uma resposta já lida por completo).
    ...
    def parse_success_response(self, response: httpx.Response, trace: TraceContext) -> TokenResponse:
        """Lê, parseia e valida uma resposta de sucesso (HTTP 200) do
        token endpoint.

        Args:
            response: resposta HTTP 200 do token endpoint -- a checagem
                do status é responsabilidade do chamador (RF-03.1);
                este método assume que já foi confirmada.
            trace: contexto de trace W3C enviado na requisição.
        ...
        Raises:
            SmartTokenError: o corpo excede o limite de tamanho, não é
                JSON válido, não é um objeto JSON, não contém
                ``access_token``, ou o campo ``expires_in`` da resposta é
        ...
        try:
            ...
            raise SmartTokenError(
                f"Resposta do token endpoint não é JSON válido (traceId={trace.trace_id}).",
                exc,
            )
        if not isinstance(parsed, dict):
            raise SmartTokenError(f"Resposta do token endpoint não é um objeto JSON (traceId={trace.trace_id}).")
...
def sanitize_expires_in(raw_expires_in: object, trace: TraceContext | None = None) -> int:
    ...
    if raw_expires_in is None:
        # Ausência é o caso esperado quando o servidor simplesmente não
