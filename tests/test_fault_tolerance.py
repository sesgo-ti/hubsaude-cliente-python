...
def _config(assertion_ttl_seconds: int, max_retries: int) -> FaultToleranceConfig:
...
def test_valid_values_are_preserved() -> None:
    config = _config(assertion_ttl_seconds=120, max_retries=5)
...
def test_invalid_assertion_ttl_falls_back_to_default(assertion_ttl_seconds: int) -> None:
    config = _config(assertion_ttl_seconds=assertion_ttl_seconds, max_retries=3)
...
def test_invalid_max_retries_falls_back_to_default(max_retries: int) -> None:
    config = _config(assertion_ttl_seconds=60, max_retries=max_retries)
    assert config.max_retries == DEFAULT_MAX_RETRIES


def test_is_frozen_dataclass() -> None:
    config = _config(assertion_ttl_seconds=60, max_retries=3)
